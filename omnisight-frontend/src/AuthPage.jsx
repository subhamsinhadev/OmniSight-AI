...
const AuthPage = () => {
  ...
  //  Show modal only if already logged in
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#060606] flex items-center justify-center p-6 relative overflow-hidden font-sans">
        ...
        <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">
          ...
          {!isLogin && (
            <div className="flex p-1 bg-black/40 rounded-2xl mb-8 border border-white/5">
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold ${role === 'client' ? 'bg-emerald-500 text-black' : 'text-gray-500'}`}
              >
                Delivery Partner
              </button>
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-sm font-semibold ${role === 'admin' ? 'bg-emerald-500 text-black' : 'text-gray-500'}`}
              >
                Admin
              </button>
            </div>
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 text-sm">
              {isLogin ? "Secure access to your earnings protection" : "Start your parametric income journey"}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-black/40 text-white border border-white/10"
                />
              </div>
            )}
            ...
            <button className="w-full bg-emerald-500 text-black py-4 rounded-2xl font-bold flex justify-center items-center gap-2">
      ...
      {open && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[#0f0f0f] border border-white/10 rounded-2xl p-6 w-80 text-center">
